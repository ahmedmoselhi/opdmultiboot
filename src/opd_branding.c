...
int opd_branding_is_compatible(const char* base_dir)
{
	...
	if (fd) {
		...
		pclose(fd);
		opd_log(LOG_DEBUG, "%-33s: box_type = %s",__FUNCTION__,  box_type);
	}

	//fix for buggy oe-branding support of some image
	if (strlen(box_type) != 0 && !strcmp(brand_oem,"vuplus") && strncmp(box_type,"vu",2)) {
		opd_log(LOG_DEBUG, "%-33s: buggy image... box_type:%s is invalid", __FUNCTION__, box_type);
		char buffer[255];
		strcpy(buffer,box_type);
		sprintf(box_type, "vu%s",buffer);
		opd_log(LOG_DEBUG, "%-33s: patched box_type:%s should be ok", __FUNCTION__, box_type);
	}

	if (strlen(box_type) == 0) {
		sprintf(fallback_arch_path, "%s/etc/opkg/arch.conf", base_dir);
		opd_log(LOG_DEBUG, "%-33s: fallback to %s parsing", __FUNCTION__, fallback_arch_path);
	...
	if (!strcmp(box_type, box_type_inflash)) {
		opd_log(LOG_DEBUG, "%-33s: box_type_inflash:%s == box_type:%s", __FUNCTION__, box_type_inflash, box_type);
		return 1;
	}
...
opd_device_item *opd_branding_read_info(const char* base_dir, const char *identifier)
{
	...
	settings_value = opd_utils_read(settings_key);
